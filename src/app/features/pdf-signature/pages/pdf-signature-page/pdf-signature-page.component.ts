...
@Component({ selector: 'app-pdf-signature-page', standalone: true, imports: [AsyncPipe, NgIf, MatSnackBarModule, MatCardModule, PdfUploadComponent, SignatureFormComponent, PdfPreviewComponent, SignatureActionsComponent, SignatureResultComponent], templateUrl: './pdf-signature-page.component.html', styleUrl: './pdf-signature-page.component.scss' })
export class PdfSignaturePageComponent {
  ...
  constructor() {
    this.error$.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((error: string | null) => {
      if (error) this.snackBar.open(error, 'Fermer', { duration: 4000 });
    });
    this.signedPdf$.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((pdf: Blob | null) => {
      if (pdf) this.snackBar.open('PDF signé prêt au téléchargement.', 'OK', { duration: 3000 });
    });
  }
